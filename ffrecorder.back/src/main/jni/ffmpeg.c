...
Java_mutter_com_ffmpeg_FFmpegNative_naInitEncoder(JNIEnv *env, jclass type, jint width, jint height,
...
Java_mutter_com_ffmpeg_FFmpegNative_naEncodeVideo(JNIEnv *env, jclass type, jobject data) {
...
//                }
                av_packet_rescale_ts(pkt, oc_ctx->time_base, ofmt_ctx->streams[0]->time_base);
                pkt->stream_index = 0;
...
JNIEXPORT void JNICALL
Java_mutter_com_ffmpeg_FFmpegNative_naFinalizeEncoder(JNIEnv *env, jclass type, jboolean isAudioEnd, jboolean isVideoEnd) {
    if (isAudioEnd) {
        isAudioStreamEnd = 1;
    }
...
JNIEXPORT jint JNICALL
Java_mutter_com_ffmpeg_FFmpegNative_naEncodeAudio(JNIEnv *env, jclass type, jshortArray  data) {
...
//                                            ofmt_ctx->streams[1]->time_base);
//                }
                av_packet_rescale_ts(pkt, aoc_ctx->time_base,
                                     ofmt_ctx->streams[1]->time_base);
